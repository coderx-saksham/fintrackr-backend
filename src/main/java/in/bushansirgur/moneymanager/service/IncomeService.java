...
public class IncomeService {
    ...
    public IncomeDTO addIncome(IncomeDTO dto) {
        ...
        if (DemoUserData.isDemoEmail(profile.getEmail())) {
            return IncomeDTO.builder()
                    ...
                    .categoryName(DemoUserData.categories().stream()
                            .filter(c -> c.getId().equals(dto.getCategoryId()))
                            .map(c -> c.getName()).findFirst().orElse("Income"))
    ...
    public void deleteIncome(Long incomeId) {
        ProfileEntity profile = profileService.getCurrentProfile();
        if (DemoUserData.isDemoEmail(profile.getEmail())) {
            return; // demo data is read-only in code
        }
    ...
    public List<IncomeDTO> getLatest5IncomesForCurrentUser() {
        ProfileEntity profile = profileService.getCurrentProfile();
        if (DemoUserData.isDemoEmail(profile.getEmail())) {
            return DemoUserData.incomes().stream().limit(5).toList();
        }
    ...
    public BigDecimal getTotalIncomeForCurrentUser() {
        ProfileEntity profile = profileService.getCurrentProfile();
        if (DemoUserData.isDemoEmail(profile.getEmail())) {
            return DemoUserData.totalIncome();
        }
    ...
    public List<IncomeDTO> filterIncomes(LocalDate startDate, LocalDate endDate, String keyword, Sort sort) {
        ...
        if (DemoUserData.isDemoEmail(profile.getEmail())) {
            ...
            return DemoUserData.incomes().stream()
                    .filter(i -> !i.getDate().isBefore(startDate) && !i.getDate().isAfter(endDate))
                    .filter(i -> key.isBlank() || i.getName().toLowerCase().contains(key))
                    .toList();
        }
