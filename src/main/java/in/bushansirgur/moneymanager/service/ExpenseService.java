...
public class ExpenseService {
        ...
        if (DemoUserData.isDemoEmail(profile.getEmail())) {
            return ExpenseDTO.builder()
                    ...
                    .categoryName(DemoUserData.categories().stream()
                            ...
                            .map(c -> c.getName()).findFirst().orElse("Expense"))
    ...
    public List<ExpenseDTO> getLatest5ExpensesForCurrentUser() {
        ProfileEntity profile = profileService.getCurrentProfile();
        if (DemoUserData.isDemoEmail(profile.getEmail())) {
            return DemoUserData.expenses().stream().limit(5).toList();
        }
    ...
    public BigDecimal getTotalExpenseForCurrentUser() {
        ProfileEntity profile = profileService.getCurrentProfile();
        if (DemoUserData.isDemoEmail(profile.getEmail())) {
            return DemoUserData.totalExpense();
        }
    ...
    public List<ExpenseDTO> filterExpenses(LocalDate startDate, LocalDate endDate, String keyword, Sort sort) {
        ...
        if (DemoUserData.isDemoEmail(profile.getEmail())) {
            ...
            return DemoUserData.expenses().stream()
                    .filter(e -> !e.getDate().isBefore(startDate) && !e.getDate().isAfter(endDate))
                    .filter(e -> key.isBlank() || e.getName().toLowerCase().contains(key))
                    .toList();
        }
